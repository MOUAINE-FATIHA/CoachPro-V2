...
class Reservation
{
    ...
    public function creer(){
        ...
        $sql = "INSERT INTO reservations (id_seance, id_sportif) VALUES (?, ?)";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([$this->id_seance, $this->id_sportif]);

        $update = $this->pdo->prepare(
            "UPDATE seances SET statut = 'reservee' WHERE id = ?"
        );
    ...
    public static function reservationsSportif(PDO $pdo, $id_sportif){
        $sql = "SELECT s.date, s.heure, s.duree, c.discipline
                FROM reservations r
    ...
    public static function reservationsCoach(PDO $pdo, $id_coach){
        $sql = "SELECT s.date, s.heure, s.duree, u.nom, u.prenom
                FROM reservations r
                JOIN seances s ON r.id_seance = s.id
